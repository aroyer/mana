defmodule ExWire.Adapter.TCP do
  ...
  """
  def send_auth_message(pid, %{peer: peer, auth_data: auth_data}) do
    Logger.debug("[Network] Sending auth message to #{peer.host}")

    GenServer.cast(pid, {:send_unframed_data, auth_data})
  end

  @doc """
  Sends acknowledgement message to peer
  """
  def send_ack_message(pid, %{peer: peer, ack_data: ack_data}) do
    Logger.debug("[Network] Sending acknowledgement to #{peer.host}")

    GenServer.cast(pid, {:send_unframed_data, ack_data})
  end
  ...
  def init(state = %{is_outbound: true, peer: _peer}) do
    ...
    send_auth_message(self(), new_state)
  ...
  @doc """
  Handle info will handle when we have inbound communication from a peer node.

  If we have already performed the handshake, then we should have `secrets` defined. In
  that case, we simply need to handle packets as they come along.
  ...
  defp handle_auth_message_received(data, state) do
    ...
    case Handshake.try_handle_auth(
      ...
      {:ok, ack_data, secrets} ->
        ...
        send_ack_message(self(), %{peer: peer, ack_data: ack_data})
  ...
  def generate_auth_credentials(state = %{peer: peer}) do
